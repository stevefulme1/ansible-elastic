"""Unit tests for stevefulme1.elastic.elastic_snapshot_repository module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.elastic.plugins.modules.elastic_snapshot_repository"
...
class TestCreateSnapshotRepository:
    """Tests for creating a snapshot_repository."""
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        params = {
            "repo_name": "full-snapshot_repository",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("snapshot_repository", params)
        mock_api_client.create.assert_called_once_with("snapshot_repository", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("snapshot_repository", {"repo_name": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "snapshot_repository": {}}
        else:
            result = mock_api_client.create("snapshot_repository", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateSnapshotRepository:
    """Tests for updating a snapshot_repository."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("snapshot_repository", "res-123", {"repo_name": "updated"})
        assert result["repo_name"] == "test-snapshot_repository-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"repo_name": existing_resource["repo_name"]}
        current = {"repo_name": existing_resource["repo_name"]}
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("snapshot_repository", "bad-id", {})


class TestDeleteSnapshotRepository:
    """Tests for deleting a snapshot_repository."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("snapshot_repository", "res-123")
        mock_api_client.delete.assert_called_once_with("snapshot_repository", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("snapshot_repository", "missing-id")
    ...
    def test_delete_api_error(self):
        ...
        with pytest.raises(Exception, match="403 Forbidden"):
            client.delete("snapshot_repository", "res-123")


class TestGetSnapshotRepository:
    """Tests for getting a snapshot_repository."""
    ...
    def test_get_nonexistent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        result = mock_api_client.get("snapshot_repository", "nonexistent")
        assert result is None
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("snapshot_repository", "res-123")


class TestListSnapshotRepository:
    """Tests for listing snapshot_repository resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"repo_name": "1", "repo_name": "first"},
            {"repo_name": "2", "repo_name": "second"},
        ]
        result = mock_api_client.list("snapshot_repository")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"repo_name": "1", "repo_name": "match"}]
        result = mock_api_client.list("snapshot_repository", filters={"repo_name": "match"})
        assert len(result) == 1


class TestIdempotencySnapshotRepository:
    """Tests for idempotent behavior of snapshot_repository."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("snapshot_repository", "res-123")
        desired_params = {"repo_name": current["repo_name"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["repo_name"] != current["repo_name"]
        assert changed is False

    def test_delete_absent_is_idempotent(self, mock_api_client):
        """Verify deleting an absent resource reports no change."""
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("snapshot_repository", "missing") is not None
        assert exists is False


class TestErrorHandlingSnapshotRepository:
    """Tests for error handling in snapshot_repository."""
    ...
    def test_auth_failure(self):
        ...
        with pytest.raises(Exception, match="401 Unauthorized"):
            client.create("snapshot_repository", {})
    ...
    def test_rate_limit(self):
        ...
        with pytest.raises(Exception, match="429"):
            client.list("snapshot_repository")
