...
def main():
    ...
    if state == "present":
        existing = None
        if resource_id:
            existing = client.get("index_template", resource_id)
        elif module.params.get("name"):
            candidates = client.list("index_template", {{"name": module.params["name"]}})
            if candidates:
                existing = candidates[0]

        if existing:
            if module.check_mode:
                module.exit_json(changed=False, index_template=existing)
            result = client.update("index_template", resource_id or existing.get("id", ""), module.params)
            module.exit_json(changed=True, index_template=result)
        else:
            if module.check_mode:
                module.exit_json(changed=True)
            result = client.create("index_template", module.params)
            module.exit_json(changed=True, index_template=result)
    else:
        existing = None
        if resource_id:
        ...
        if module.check_mode:
            module.exit_json(changed=True)
        client.delete("index_template", resource_id)
        module.exit_json(changed=True)
