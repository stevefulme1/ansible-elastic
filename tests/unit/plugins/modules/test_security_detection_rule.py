...
MODULE_PATH = "ansible_collections.stevefulme1.elastic.plugins.modules.security_detection_rule"
CLIENT_PATH = "ansible_collections.stevefulme1.elastic.plugins.module_utils.api_client"


def _build_resource(**overrides):
    """Return a mock security_detection_rule resource dict."""
    base = {
        "id": "res-123",
        "rule_id": "res-123",
        "name": "test-name",
...
def resource_args(module_args):
    """Module args for security_detection_rule operations."""
    module_args.update({
        "state": "present",
...
class TestGetCurrentState:
    ...
    def test_returns_matching_resource_by_name(self, resource_args):
        """get_current_state returns existing resource when found by name."""
        resource_args["rule_id"] = None
        resource_args["name"] = "test-name"
        mock_client = MagicMock()
        existing = _build_resource(name="test-name")
        mock_client.get.return_value = {"data": [existing]}

        mock_module = MagicMock()
        mock_module.params = resource_args

        from ansible_collections.stevefulme1.elastic.plugins.modules.security_detection_rule import get_current_state
        result = get_current_state(mock_client, mock_module)
        assert result is not None

    def test_returns_matching_resource_by_id(self, resource_args):
        """get_current_state returns existing resource when found by rule_id."""
        resource_args["rule_id"] = "rule-123"
        mock_client = MagicMock()
        mock_client.get.return_value = _build_resource(id="rule-123")
...
class TestNeedsUpdate:
    ...
    def test_returns_true_when_no_current(self):
        ...
        from ansible_collections.stevefulme1.elastic.plugins.modules.security_detection_rule import needs_update
...
class TestBuildPayload:
    ...
    def test_builds_payload_from_params(self, resource_args):
        ...
        from ansible_collections.stevefulme1.elastic.plugins.modules.security_detection_rule import build_payload
...
class TestCreate:
    ...
    def test_create_sets_changed(self, mock_ansible_cls, mock_client_cls, resource_args):
        ...
        with patch(f"{MODULE_PATH}.get_current_state", return_value=None):
            from ansible_collections.stevefulme1.elastic.plugins.modules.security_detection_rule import main
            main()
