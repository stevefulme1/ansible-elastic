"""Unit tests for stevefulme1.elastic.kibana_space module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.elastic.plugins.modules.kibana_space"
CLIENT_PATH = "ansible_collections.stevefulme1.elastic.plugins.module_utils.api_client"


def _build_resource(**overrides):
    """Return a mock kibana_space resource dict."""
    base = {
        "id": "res-123",
        "space_id": "res-123",
        "name": "test-name",
...
def resource_args(module_args):
    """Module args for kibana_space operations."""
    module_args.update({
        "state": "present",
...
class TestGetCurrentState:
    ...
    def test_returns_none_without_lookup(self, resource_args):
        ...
        from ansible_collections.stevefulme1.elastic.plugins.modules.kibana_space import get_current_state
...
class TestNeedsUpdate:
    ...
    def test_returns_true_when_no_current(self):
        ...
        from ansible_collections.stevefulme1.elastic.plugins.modules.kibana_space import needs_update
...
class TestBuildPayload:
    ...
    def test_builds_payload_from_params(self, resource_args):
        ...
        from ansible_collections.stevefulme1.elastic.plugins.modules.kibana_space import build_payload
...
class TestCreate:
    ...
    def test_create_sets_changed(self, mock_ansible_cls, mock_client_cls, resource_args):
        ...
        with patch(f"{MODULE_PATH}.get_current_state", return_value=None):
            from ansible_collections.stevefulme1.elastic.plugins.modules.kibana_space import main
            main()
...
class TestUpdate:
    ...
    def test_update_when_changed(self, mock_ansible_cls, mock_client_cls, resource_args):
        ...
        resource_args["space_id"] = "new-value"
        mock_module = MagicMock()
        mock_module.params = resource_args
        mock_module.check_mode = False
        mock_ansible_cls.return_value = mock_module
        ...
        mock_client = MagicMock()
        mock_client.put.return_value = _build_resource(space_id="new-value")
        mock_client_cls.return_value = mock_client

        existing = _build_resource(space_id="old-value")
        with patch(f"{MODULE_PATH}.get_current_state", return_value=existing), \
