...
def main():
    ...
    if state == "present":
        existing = None
        if resource_id:
            existing = client.get("ccr_auto_follow", resource_id)
        elif module.params.get("name"):
            candidates = client.list("ccr_auto_follow", {{"name": module.params["name"]}})
            if candidates:
                existing = candidates[0]

        if existing:
            if module.check_mode:
                module.exit_json(changed=False, ccr_auto_follow=existing)
            result = client.update("ccr_auto_follow", resource_id or existing.get("id", ""), module.params)
