...
"""Unit tests for stevefulme1.elastic.fleet_agent_policy_info module."""
...
from ansible_collections.stevefulme1.elastic.plugins.modules import fleet_agent_policy_info
...
SAMPLE_POLICY_LIST = [
    ...
    {"id": "policy-456", "name": "Other Policy"},
]
...
class TestFetchSingle:
    def test_found(self):
        ...
        result = fleet_agent_policy_info.fetch_single(client, "policy-123")
    ...
    def test_not_found(self):
        ...
        result = fleet_agent_policy_info.fetch_single(client, "nonexistent")
...
class TestFetchList:
    def test_list_all(self):
        ...
        result = fleet_agent_policy_info.fetch_list(client)
        client.get.assert_called_once_with("/api/fleet/agent_policies")
        assert len(result) == 2
    ...
    def test_list_client_error(self):
        ...
        with pytest.raises(ClientError):
            fleet_agent_policy_info.fetch_list(client)


class TestMainSingle:
    @patch("ansible_collections.stevefulme1.elastic.plugins.modules.fleet_agent_policy_info.Client")
    def test_get_single_policy(self, mock_client_cls):
        ...
        args = dict(BASE_ARGS)
        args["policy_id"] = "policy-123"
        set_module_args(args)

        with pytest.raises(SystemExit) as exc_info:
            fleet_agent_policy_info.main()

        assert exc_info.value.code == 0

    @patch("ansible_collections.stevefulme1.elastic.plugins.modules.fleet_agent_policy_info.Client")
    def test_get_missing_policy(self, mock_client_cls):
        mock_client = MagicMock()
        mock_client_cls.return_value = mock_client
...
class TestMainList:
    @patch("ansible_collections.stevefulme1.elastic.plugins.modules.fleet_agent_policy_info.Client")
    def test_list_all_policies(self, mock_client_cls):
        mock_client = MagicMock()
        mock_client_cls.return_value = mock_client
