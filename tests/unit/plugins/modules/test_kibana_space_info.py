...
"""Unit tests for stevefulme1.elastic.kibana_space_info module."""
...
from ansible_collections.stevefulme1.elastic.plugins.modules import kibana_space_info
...
class TestFetchSingle:
    def test_found(self):
        client = MagicMock()
        client.get.return_value = SAMPLE_SPACE

        result = kibana_space_info.fetch_single(client, "marketing")
    ...
    def test_not_found(self):
        ...
        result = kibana_space_info.fetch_single(client, "nonexistent")
...
class TestFetchList:
    def test_list_all(self):
        client = MagicMock()
        client.get.return_value = SAMPLE_SPACE_LIST

        result = kibana_space_info.fetch_list(client)
        client.get.assert_called_once_with("/api/spaces/space")
        assert len(result) == 2
...
class TestMainSingle:
    @patch("ansible_collections.stevefulme1.elastic.plugins.modules.kibana_space_info.Client")
    def test_get_single_space(self, mock_client_cls):
        mock_client = MagicMock()
        mock_client_cls.return_value = mock_client
        mock_client.get.return_value = SAMPLE_SPACE
        mock_client.headers = {}

        args = dict(BASE_ARGS)
        args["space_id"] = "marketing"
        set_module_args(args)

        with pytest.raises(SystemExit) as exc_info:
            kibana_space_info.main()

        assert exc_info.value.code == 0

    @patch("ansible_collections.stevefulme1.elastic.plugins.modules.kibana_space_info.Client")
    def test_get_missing_space(self, mock_client_cls):
        mock_client = MagicMock()
        mock_client_cls.return_value = mock_client
...
class TestMainList:
    @patch("ansible_collections.stevefulme1.elastic.plugins.modules.kibana_space_info.Client")
    def test_list_all_spaces(self, mock_client_cls):
        mock_client = MagicMock()
        mock_client_cls.return_value = mock_client
        mock_client.get.return_value = SAMPLE_SPACE_LIST
        mock_client.headers = {}
