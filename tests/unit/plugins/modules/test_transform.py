"""Unit tests for stevefulme1.elastic.transform module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.elastic.plugins.modules.transform"
CLIENT_PATH = "ansible_collections.stevefulme1.elastic.plugins.module_utils.api_client"


def _build_resource(**overrides):
    """Return a mock transform resource dict."""
    base = {
        "id": "res-123",
        "dest": "test-dest",
        "source": "test-source",
...
def resource_args(module_args):
    """Module args for transform operations."""
    module_args.update({
        "state": "present",
...
class TestNeedsUpdate:
    ...
    def test_returns_true_when_no_current(self):
        ...
        from ansible_collections.stevefulme1.elastic.plugins.modules.transform import needs_update
...
class TestBuildPayload:
    ...
    def test_builds_payload_from_params(self, resource_args):
        ...
        from ansible_collections.stevefulme1.elastic.plugins.modules.transform import build_payload
...
class TestCreate:
    ...
    def test_create_sets_changed(self, mock_ansible_cls, mock_client_cls, resource_args):
        ...
        with patch(f"{MODULE_PATH}.get_current_state", return_value=None):
            from ansible_collections.stevefulme1.elastic.plugins.modules.transform import main
            main()
...
class TestUpdate:
    ...
    def test_update_when_changed(self, mock_ansible_cls, mock_client_cls, resource_args):
        ...
        mock_client = MagicMock()
        mock_client.put.return_value = _build_resource(dest="new-value")
        mock_client_cls.return_value = mock_client

        existing = _build_resource(dest="old-value")
        with patch(f"{MODULE_PATH}.get_current_state", return_value=existing), \
